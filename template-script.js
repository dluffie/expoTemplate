...
async function captureCanvas(scale) {
    ...
    const uiEls = document.querySelectorAll('.sec-del,.sec-drag,.sec-width,.sec-resize-r,.sec-resize-b,.sec-resize-br,.dia-copy,.dia-del,.dia-drag,.ms-del,.ms-handle,.shape-toolbar,.ci-handle,.ci-del,.add-sec-row')
...
async function exportPPTX() {
    showOverlay('📊', 'Building PPTX…', 'Creating editable PowerPoint')
    try {
        const pptx = new PptxGenJS()
        // A4 portrait in inches (210mm × 297mm)
        const slideW = 8.27, slideH = 11.69
        pptx.defineLayout({ name: 'A4', width: slideW, height: slideH })
        pptx.layout = 'A4'

        const page = document.querySelector('.page')
        const pageW = page.offsetWidth
        const pageH = page.scrollHeight
        const scaleX = slideW / pageW
        const scaleY = slideH / pageH

        function px2in(px, axis) { return px * (axis === 'x' ? scaleX : scaleY) }

        const slide = pptx.addSlide()

        const rootStyle = getComputedStyle(document.documentElement)
        const hdr1 = rootStyle.getPropertyValue('--hdr1').trim() || '#1a2744'
        const hdr2 = rootStyle.getPropertyValue('--hdr2').trim() || '#2c4a8c'
        const gold = rootStyle.getPropertyValue('--gold').trim() || '#c8973a'
        const pageBg = rootStyle.getPropertyValue('--page-bg').trim() || '#fdf8f0'
        const tagColor1 = rootStyle.getPropertyValue('--tag1').trim() || '#e8a96a'
        const cardBorder = rootStyle.getPropertyValue('--card-border').trim() || '#2c4a8c'
        const contentTxt = rootStyle.getPropertyValue('--content-txt').trim() || '#2a2a3e'

        // Page background
        slide.background = { color: pageBg.replace('#', '') }
        ...
        if (header) {
            ...
            const hx = 0, hy = 0
            const hw = slideW, hh = px2in(hRect.height, 'y')
            slide.addShape(pptx.ShapeType.rect, {
                x: hx, y: hy, w: hw, h: hh,
                fill: { color: hdr1.replace('#', '') }
            })
            // College name
            const collegeName = document.querySelector('.college-name')
            if (collegeName) {
                slide.addText(collegeName.innerText, {
                    x: 0.5, y: 0.15, w: slideW - 1, h: 0.7,
                    fontSize: 14, fontFace: 'Arial', bold: true, color: 'FFFFFF',
                    align: 'center', valign: 'middle'
                })
            }
            // Association name
            const assocName = document.querySelector('.assoc-name')
            if (assocName) {
                slide.addText(assocName.innerText, {
                    x: 0.5, y: 0.75, w: slideW - 1, h: 0.25,
                    fontSize: 8, fontFace: 'Arial', bold: true, color: gold.replace('#', ''),
                    align: 'center', valign: 'middle', charSpacing: 3
                })
            }
            // CESA + Presents + EXPO + Date line
            const cesaBadge = document.querySelector('.cesa-badge')
            const expoTitle = document.querySelector('.expo-title')
            const dateBadge = document.querySelector('.date-badge')
            let headerLine = ''
            if (cesaBadge) headerLine += cesaBadge.innerText + '  '
            headerLine += '– Presents –  '
            if (expoTitle) headerLine += expoTitle.innerText + '  '
            if (dateBadge) headerLine += dateBadge.innerText
            slide.addText(headerLine, {
                x: 0.3, y: hh - 0.45, w: slideW - 0.6, h: 0.4,
        ...
        if (titleBand) {
            const tbRect = titleBand.getBoundingClientRect()
            const pRect = page.getBoundingClientRect()
            const ty = px2in(tbRect.top - pRect.top, 'y')
            const th = px2in(tbRect.height, 'y')
            slide.addShape(pptx.ShapeType.rect, {
                x: 0, y: ty, w: slideW, h: th,
                fill: { color: hdr1.replace('#', '') }
            })
            const titleLabel = document.querySelector('.title-label')
            const titleText = document.querySelector('.title-text')
            const textParts = []
            if (titleLabel) textParts.push({ text: titleLabel.innerText + '\n', options: { fontSize: 7, color: gold.replace('#', ''), charSpacing: 3, bold: true } })
            if (titleText) textParts.push({ text: titleText.innerText, options: { fontSize: 14, color: 'FFFFFF', bold: true } })
            slide.addText(textParts, {
                x: 0.3, y: ty, w: slideW - 0.6, h: th,
        ...
        for (const sec of sections) {
            ...
            const sx = px2in(sRect.left - pRect.left, 'x')
            const sy = px2in(sRect.top - pRect.top, 'y')
            const sw = px2in(sRect.width, 'x')
            const sh = px2in(sRect.height, 'y')

            // Section border/bg
            slide.addShape(pptx.ShapeType.rect, {
                x: sx, y: sy, w: sw, h: sh,
                fill: { color: 'FFFFFF' },
                line: { color: cardBorder.replace('#', ''), width: 1.5 },
                rectRadius: 0.04
            })

            // Tag bar
            const stag = sec.querySelector('.stag')
            if (stag) {
                const stagR = stag.getBoundingClientRect()
                const stagH = px2in(stagR.height, 'y')
                slide.addShape(pptx.ShapeType.rect, {
                    x: sx, y: sy, w: sw, h: stagH,
                    fill: { color: tagColor1.replace('#', '') }
                })
                slide.addText(stag.innerText, {
                    x: sx + 0.05, y: sy, w: sw - 0.1, h: stagH,
            ...
            // Content text
            const scont = sec.querySelector('.scont')
            if (scont && scont.innerText.trim()) {
                const scontR = scont.getBoundingClientRect()
                const cy = px2in(scontR.top - pRect.top, 'y')
                const ch = px2in(scontR.height, 'y')
                slide.addText(scont.innerText, {
                    x: sx + 0.05, y: cy, w: sw - 0.1, h: ch,
                    fontSize: 7.5, fontFace: 'Arial', color: contentTxt.replace('#', ''),
                    valign: 'top', paraSpaceAfter: 2, lineSpacing: 12
                })
            }

            // Diagrams — render as images
            const diagrams = sec.querySelectorAll('.diagram-box svg')
            for (const svg of diagrams) {
                try {
                    const box = svg.closest('.diagram-box')
                    const boxR = box.getBoundingClientRect()
                    // Hide controls for capture
                    const ctrls = box.querySelectorAll('.shape-toolbar,.dia-copy,.dia-del,.dia-drag')
                    ctrls.forEach(c => c.style.display = 'none')
                    const diaCanvas = await html2canvas(box, {
                        scale: 2, useCORS: true, allowTaint: true, backgroundColor: '#fafbfe', logging: false
                    })
                    ctrls.forEach(c => c.style.display = '')
                    const imgData = diaCanvas.toDataURL('image/png')
                    const dx = px2in(boxR.left - pRect.left, 'x')
                    const dy = px2in(boxR.top - pRect.top, 'y')
                    const dw = px2in(boxR.width, 'x')
                    const dh = px2in(boxR.height, 'y')
                    slide.addImage({ data: imgData, x: dx, y: dy, w: dw, h: dh })
                } catch (e) { console.warn('Diagram export failed:', e) }
            }
        }
        ...
        if (footer) {
            ...
            const fy = px2in(fRect.top - pRect.top, 'y')
            const fh = px2in(fRect.height, 'y')
            slide.addText(footer.innerText, {
                x: 0, y: fy, w: slideW, h: fh,
                fontSize: 6.5, fontFace: 'Arial', color: 'AAAAAA',
                align: 'center', valign: 'middle'
            })
        }

        await pptx.writeFile({ fileName: 'project-expo-poster.pptx' })
        showOverlay('✅', 'PPTX Saved!', 'Open in PowerPoint or Google Slides to edit')
        setTimeout(hideOverlay, 2500)
    } catch (err) {
        console.error(err)
        showOverlay('❌', 'Export failed', err.message)
...
/* ══════════════════════════════════════
   DIAGRAM COPY-PASTE + DRAG + DELETE
══════════════════════════════════════ */
...
function injectDiagramControls() {
    document.querySelectorAll('.diagram-box').forEach(box => {
        // Add delete button
        if (!box.querySelector('.dia-del')) {
            const del = document.createElement('button')
            del.className = 'dia-del'
            del.title = 'Delete diagram'
            del.textContent = '✕'
            del.addEventListener('click', function (e) {
                e.stopPropagation()
                if (confirm('Delete this diagram?')) box.remove()
            })
            box.appendChild(del)
        }
        // Add drag handle
        if (!box.querySelector('.dia-drag')) {
            const handle = document.createElement('button')
            handle.className = 'dia-drag'
...
/* Diagram drag within parent */
document.addEventListener('mousedown', function (e) {
    const handle = e.target.closest('.dia-drag')
    if (!handle) return
    const box = handle.closest('.diagram-box')
    if (!box) return
    e.preventDefault()
    const parent = box.parentElement
    const parentRect = parent.getBoundingClientRect()
    // Switch to relative positioning for movement
...
document.addEventListener('mousemove', function (e) {
    if (!diaDrag) return
    const dx = e.clientX - diaDrag.startX
    const dy = e.clientY - diaDrag.startY
    diaDrag.el.style.left = (diaDrag.startLeft + dx) + 'px'
    diaDrag.el.style.top = (diaDrag.startTop + dy) + 'px'
})
...
document.addEventListener('paste', function (e) {
    ...
    // Re-inject toolbars and controls on new diagram box
    injectShapeToolbars()
    injectDiagramCopyButtons()
    injectDiagramControls()
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    injectShapeToolbars()
    // Inject diagram copy buttons and controls
    injectDiagramCopyButtons()
    injectDiagramControls()
